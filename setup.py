...
for k, v in settings['graphics'].items():
    filename = v['source_file']
    path = os.path.join(resource_dir, filename)

    # Don't download if it's already there
    if os.path.exists(path):
        info(f"File {filename} already exists for graphic {k}. Skipping download")
        continue

    link = v.get('web_source', None)
    if not link:
        error(f"No web source exists for graphic {k}. Check if you're missing {filename} ?")
        continue
